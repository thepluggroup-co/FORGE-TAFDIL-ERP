...
describe('C6 — PATCH /api/devis/:id/statut : transition devis', () => {
  ...
  it('permet de valider directement un brouillon au nom du client', async () => {
    vi.mocked(supabase.from).mockReturnValueOnce(
      mkChain({ data: { ...DEVIS, statut: 'brouillon' }, error: null }) as never,
    )
    vi.mocked(supabase.from).mockReturnValueOnce(
      mkChain({
        data: {
          ...
          approuve_at: '2026-06-02T10:00:00.000Z',
          devis_lignes: [],
          clients: null,
        },
        error: null,
    ...
    expect(res.status).toBe(200)
    const body = await res.json() as { statut: string; approuve_par_client: boolean }
    expect(body.statut).toBe('accepte')
    expect(body.approuve_par_client).toBe(true)
  })

  it('permet d’annuler directement un devis en attente', async () => {
    vi.mocked(supabase.from).mockReturnValueOnce(
      mkChain({ data: { ...DEVIS, statut: 'envoye' }, error: null }) as never,
    )
    vi.mocked(supabase.from).mockReturnValueOnce(
      mkChain({
        data: { ...DEVIS, statut: 'refuse', approuve_par_client: false, devis_lignes: [], clients: null },
        error: null,
      }) as never,
    )
    ...
    expect(body.approuve_par_client).toBe(false)
  })
