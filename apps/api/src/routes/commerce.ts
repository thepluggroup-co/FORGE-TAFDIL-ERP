...
router.patch('/devis/:id/statut', requirePermission('COMMERCIAL', 'VALIDATE'), zValidator('json', z.object({
  ...
  const { data, error } = await db
    ...
    .update({
      statut,
      updated_at: now,
      // A staff decision in the ERP is made on behalf of the client. Keep the
      // same approval fields as the public approval flow so an accepted quote
      // can immediately be converted into an ERP order.
      ...(statut === 'accepte' || statut === 'refuse' ? {
        approuve_par_client: statut === 'accepte',
        approuve_at:         now,
        commentaire_client:  null,
        token_approbation:   null,
        token_expires_at:    null,
...
router.post('/devis/:id/transformer-commande', requirePermission('COMMERCIAL', 'CREATE'), async (c) => {
  ...
  // Vérifier le statut — seule une validation permet de créer une commande.
