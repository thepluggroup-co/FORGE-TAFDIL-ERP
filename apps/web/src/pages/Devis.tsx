...
import {
  Plus, Trash2, MessageCircle, ArrowRightLeft,
  FileText, Loader2, Check, X, ChevronLeft, ChevronRight,
  Edit2, Eye, Clock, CheckCircle, OctagonX,
  ShieldCheck,
} from 'lucide-react'
...
import {
  ...
  useUpdateStatutDevis, useTransformerDevis,
      ...
      {expires.length > 0 && (
        ...
        >
          ...
          <p className="text-sm text-red-700">
             <span className="font-bold">{expires.length} devis expiré(s)</span> — ils ne peuvent plus être validés.
          </p>
        </motion.div>
      )}
...
function DevisDetailPanel({
  devis, onClose, onEdit, onDelete, onUpdated, role,
}: {
  devis: DevisRecord
  onClose: () => void
  onEdit: () => void
  onDelete: () => void
  onUpdated: (devis: DevisRecord) => void
  role: string | null
}) {
  const transformer        = useTransformerDevis()
  const updateStatut       = useUpdateStatutDevis()
  const { hasPermission }  = usePermissions()
  const [decision, setDecision] = useState<'accepte' | 'refuse' | null>(null)
  ...
  const canDecide      = canAdmin && ['brouillon', 'envoye'].includes(statut)
  const canTransformer  = hasPermission('COMMERCIAL', 'CREATE') && statut === 'accepte'
  const sc             = STATUT_COLORS[statut] ?? STATUT_COLORS.brouillon

  const submitDecision = () => {
    if (!decision) return
    updateStatut.mutate(
      { id: devis.id as string, statut: decision },
      {
        onSuccess: (updated) => {
          onUpdated(updated as DevisRecord)
          setDecision(null)
        },
      },
    )
    ...
    <SlideOver isOpen={true} onClose={onClose} title={`Devis ${devis.reference as string}`} width="lg">
      <div className="space-y-5">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {approuve && (
                <span className="flex items-center gap-1 text-xs font-semibold text-green-700 bg-green-50 px-2 py-0.5 rounded-full">
                   <ShieldCheck className="h-3 w-3" /> Validé pour le client
                </span>
              )}
              {devis.expire && (
        ...
        <div className="border-t border-gray-100 pt-4 space-y-2">
           ...
           {/* Décision ERP prise au nom du client */}
           {canDecide && (
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
               <button
                 onClick={() => setDecision('accepte')}
                 disabled={updateStatut.isPending}
                 className="flex items-center justify-center gap-2 px-3 py-3 rounded-xl border-2 border-green-200 bg-green-50 hover:bg-green-100 transition-colors text-sm font-semibold text-green-700 disabled:opacity-50"
               >
               ...
               <button
                 ...
                 disabled={updateStatut.isPending}
                 className="flex items-center justify-center gap-2 px-3 py-3 rounded-xl border-2 border-red-200 bg-red-50 hover:bg-red-100 transition-colors text-sm font-semibold text-red-700 disabled:opacity-50"
               >
          ...
          {canTransformer && (
            ...
            >
              ...
              <div>
               ...
               <p className="text-xs text-purple-500">Devis validé pour le client ✓</p>
              </div>
            </button>
          )}
         ...
         <Modal
           ...
           title={decision === 'accepte' ? 'Valider ce devis ?' : 'Annuler ce devis ?'}
           size="sm"
         >
           <div className="space-y-4">
             <p className="text-sm text-gray-600">
               {decision === 'accepte'
                 ? 'Cette validation est enregistrée au nom du client. Le devis pourra ensuite être transformé en commande ERP.'
                 : 'Le devis sera marqué comme annulé et ne pourra plus être transformé en commande.'}
             </p>
             <div className="flex gap-3">
               <Button variant="ghost" className="flex-1" onClick={() => setDecision(null)}>Retour</Button>
               <button
                 onClick={submitDecision}
                 disabled={updateStatut.isPending}
                 className={`flex-1 flex items-center justify-center gap-2 py-2 text-sm font-semibold rounded-lg text-white disabled:opacity-50 transition-colors ${
                   decision === 'accepte' ? 'bg-green-700 hover:bg-green-800' : 'bg-red-600 hover:bg-red-700'
                 }`}
               >
                 {updateStatut.isPending && <Loader2 className="h-3.5 w-3.5 animate-spin" />}
                 {decision === 'accepte' ? 'Confirmer la validation' : 'Confirmer l’annulation'}
               </button>
             </div>
           </div>
         </Modal>
      ...
      render: (v, row) => (
        <div className="flex items-center gap-1.5">
          ...
          {(row.approuve_par_client as boolean) && (
             <ShieldCheck className="h-3.5 w-3.5 text-green-500 shrink-0" title="Validé pour le client" />
          )}
        </div>
      ),
    },
      ...
      {selectedDevis && (
        <DevisDetailPanel
          ...
          onDelete={() => { setDeleteId(selectedDevis.id as string); setSelectedDevis(null) }}
           onUpdated={(updated) => setSelectedDevis(updated)}
        />
      )}
